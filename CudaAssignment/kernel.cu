...
//First half of the threads of a block load their values and the values of 
//the other half, including halo rows/cols
__global__ void JacobiStep(const float *oldMatrix, float *newMatrix)
{
	...
	if (getSharedIndex(thx,thy) < getSharedIndex(blockDim.x,blockDim.y/2-1))
	{
		...
		if (thx == 0) 	   {
			...
			aux[getSharedIndex(thx-1,thy+8)] = 
										oldMatrix[getGlobalIndex(thx-1,thy+8)];
		}
		//top
		if  (thy == 0) 
		...
		//bot
		if (thy == blockDim.y/2-1)			
			aux[getSharedIndex(thx,thy+9)]=oldMatrix[getGlobalIndex(thx,thy+9)];	
		
	}
	__syncthreads();	
	newMatrix[getGlobalIndex(thx,thy)] = 0.25*( aux[rightIndex]+aux[topIndex]+
